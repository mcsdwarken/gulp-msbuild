...
var detectMsBuild15Dir = function (pathRoot, vsVersion) {
  var wherePath = msBuildFromWhere(pathRoot) || '';
  if (wherePath.length > 0) {
    return wherePath;
  }
  vsVersion = vsVersion || "2017";
  var vsPath = path.join(pathRoot, 'Microsoft Visual Studio', vsVersion);
  var possibleFolders = ['BuildTools', 'Enterprise', 'Professional', 'Community'];

  for (var index = 0; index < possibleFolders.length; index++) {
    try {
      var folderPath = path.join(vsPath, possibleFolders[index]);
...
var detectMsBuildVersion = function(msBuildPath){
  return /[\/\\]2017[\/\\]/.test(msBuildPath) ? 15.0 : "Current";
}

var autoDetectVersion = function (pathRoot) {
  // Try to detect MSBuild 15.0.
  var msbuild15Dir = detectMsBuild15Dir(pathRoot, "2019");
  if (msbuild15Dir) {
    return [msbuild15Dir, detectMsBuildVersion(msbuild15Dir)];
  }
  msbuild15Dir = detectMsBuild15Dir(pathRoot, "2017");
  if (msbuild15Dir) {
    return [msbuild15Dir, detectMsBuildVersion(msbuild15Dir)];
  }
...
module.exports.find = function (options) {
  ...
  if (options.toolsVersion === 'auto') {
    ...
    if (options.toolsVersion === 15.0 || options.toolsVersion === "Current") {
      var msbuildDir = detectMsBuild15Dir(pathRoot, options.toolsVersion === "Current" ? "2019" : "2017");
      if (msbuildDir) {
